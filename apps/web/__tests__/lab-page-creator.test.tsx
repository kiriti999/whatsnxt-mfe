...
import LabPageCreator from '@/components/LabPageCreator';

// Mock API calls
vi.mock('../../app/apis/lab.api', () => ({
  default: {
    createLabPage: vi.fn((labId, data) =>
      Promise.resolve({
        id: 'new-page-id',
        labId,
...
describe('LabPageCreator', () => {
  ...
  it('should render the stepper and create a lab page', async () => {
    ...
    await waitFor(() =>
      expect(labApi.createLabPage).toHaveBeenCalledWith(
        mockLabId,
    ...
    await waitFor(() =>
      expect(mockOnPageCreated).toHaveBeenCalledWith(expect.objectContaining({ id: 'new-page-id' }))
    );
  });
