import { useContext } from "react";
import { TiptapManageContext } from '../../context/TiptapManageContext';
import { Progress } from '@mantine/core';

const colors = ['blue', 'green', 'pink', 'orange', 'purple', 'yellow'];

const CourseUploadAssetProgress = () => {
    const { progressList, isAssetsUploading } = useContext(TiptapManageContext)

    return (
        <div style={{ marginTop: isAssetsUploading ? "" : "16px" }}>
            {progressList && progressList.length > 0 ? <>
                <Progress.Root size="xl" transitionDuration={200} >
                    {progressList.map((e, index) => (<>
                        <Progress.Section key={e?.fileName} value={e.progress} animated color={colors[index % colors.length]}>
                            <Progress.Label key={index}>{e?.fileName}</Progress.Label>
                        </Progress.Section>
                    </>))}
                </Progress.Root>
            </> : <></>}
            {/* <Progress color="red" size="xl" value={80} animated /> */}
        </div>
    );
};

export default CourseUploadAssetProgress





