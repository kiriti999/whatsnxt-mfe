...
import { Button, Group, Stepper, Box, Title, Text } from '@mantine/core';
...
const LabPageCreator: React.FC<LabPageCreatorProps> = ({ labId, onPageCreated }) => {
  ...
  const createPage = async () => {
    try {
      const newPage = await labApi.createLabPage(labId, { hasQuestion: false, hasDiagramTest: false }); // Default to no content initially
      notifications.show({
        title: 'Success',
  ...
  return (
    <Box maw={800} mx="auto">
      <Title order={2} mb="md">Create Lab Pages for Lab: {labId}</Title>
      <Stepper active={active} onStepClick={setActive} allowNextStepsSelect={false}>
        <Stepper.Step label="First step" description="Page Type">
          <Title order={4}>Select Page Content Type</Title>
          <Text>Here you would choose if this page has a question, a diagram test, or both.</Text>
        ...
        <Stepper.Step label="Second step" description="Content">
          ...
          <Text>This section would contain QuestionEditor or DiagramTestEditor.</Text>
