...
import { Box, Button, Container, Divider, Group, SimpleGrid, Stack, Text, ThemeIcon, Title, UnstyledButton } from '@mantine/core';
import {
    IconArrowRight,
    IconCloud,
    IconCode,
...
const LAB_TYPE_CONFIG: Record<string, LabConfig> = {
    cloud: { icon: <IconCloud size={16} />, color: 'cyan', topClass: styles.cardTopCyan, textColor: 'var(--mantine-color-cyan-6)' },
    security: { icon: <IconShield size={16} />, color: 'red', topClass: styles.cardTopRed, textColor: 'var(--mantine-color-red-6)' },
    networking: { icon: <IconNetwork size={16} />, color: 'grape', topClass: styles.cardTopGrape, textColor: 'var(--mantine-color-grape-6)' },
    programming: { icon: <IconCode size={16} />, color: 'indigo', topClass: styles.cardTopIndigo, textColor: 'var(--mantine-color-indigo-6)' },
    architecture: { icon: <IconTerminal2 size={16} />, color: 'violet', topClass: styles.cardTopViolet, textColor: 'var(--mantine-color-violet-6)' },
    database: { icon: <IconDatabase size={16} />, color: 'orange', topClass: styles.cardTopOrange, textColor: 'var(--mantine-color-orange-6)' },
};
            ...
            <Container size="xl">
                ...
                <Stack align="center" gap={4} mb="xl" hiddenFrom="sm">
                    <Group gap={6}>
                        ...
                        <Text size="xs" fw={700} tt="uppercase" c="orange" style={{ letterSpacing: '0.08em' }}>
                            Hands-on Practice
                        </Text>
                    </Group>
                ...
                <SimpleGrid cols={{ base: 1, xs: 2, sm: 2, md: 3, lg: 4 }} spacing="md">
                    {labs.map((lab) => {
                        const { icon, color, topClass, textColor } = getLabConfig(lab.labType);
                        ...
                        return (
                            ...
                            >
                                {/* Colored top bar */}
                                <Group className={`${styles.cardTop} ${topClass}`} gap="xs" align="center">
                                    <ThemeIcon size={28} radius="sm" variant="light" color={color}>
                                        {icon}
                                    </ThemeIcon>
                                    <Text size="xs" fw={700} tt="uppercase" style={{ color: textColor }}>
                                        {lab.labType || 'Lab'}
                                    </Text>
                                </Group>
                                ...
                                <Stack gap="xs" className={styles.cardFooter}>
                                    <Group gap={6} wrap="nowrap">
                                        {hasQuestions && (
                                            <Group gap={4}>
                                                <IconListCheck size={13} color="var(--mantine-color-blue-5)" />
                                                <Text size="xs" fw={600}>{lab.questionCount}</Text>
                                                <Text size="xs" c="dimmed">MCQs</Text>
                                            </Group>
                                        )}
                                        ...
                                        {hasDiagrams && (
                                            <Group gap={4}>
                                                <IconSchema size={13} color="var(--mantine-color-violet-5)" />
                                                <Text size="xs" fw={600}>{lab.diagramTestCount}</Text>
                                                <Text size="xs" c="dimmed">Diagrams</Text>
                                            </Group>
                                        )}
                                        {!hasStats && (
                                            <Text size="xs" c="dimmed">Practice lab</Text>
                                        )}
                                    </Group>
