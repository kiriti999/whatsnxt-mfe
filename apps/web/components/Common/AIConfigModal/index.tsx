...
const API_KEY_LINKS: Record<string, { label: string; href: string }> = {
    ...
    gemini: { label: 'Google AI Studio', href: 'https://aistudio.google.com/app/apikey' },
};
...
export function AIConfigModal({
    ...
    const handleSaveKeyAndGenerate = useCallback(async () => {
        ...
        try {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                setApiKeyError(
                    response.data?.message || response.data?.error || 'Failed to save API key',
                );
            }
        } catch (err: any) {
            ...
            if (err?.response?.data?.message) {
                ...
                if (err.message.includes('Route') && err.message.includes('not found')) {
                    errorMessage = `${err.message}. Please contact support.`;
                }
            }

            if (!err?.response?.data?.message && !err?.response?.data?.error && !err?.message) {
    ...
    return (
        <Modal opened={opened} onClose={onClose} title="Configure AI Assistant" centered size="lg">
            <Stack gap="md">
                {displayError && (
                ...
                <Text size="sm" fw={500}>
                    Select your AI provider and model. Optionally add your own API key to overcome
                    rate limits.
                </Text>
