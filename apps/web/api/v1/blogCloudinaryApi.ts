...
export const CloudinaryAPI = {
  ...
  uploadFormData: async function (formData: any, type: 'image' | 'raw' | 'video' = 'image') {
    ...
    const response = await bffApiClient.post(getUploadUrl(type), formData, {
      withCredentials: false,
    });
  ...
  delete: async function (id: string) {
    const { data } = await bffApiClient.delete('/cloudinary/deleteAsset', { deleteID: id });
  ...
  deleteMultipleAssets: async function (assets: { type: string, id: string }[], accessToken: string) {
    const { data } = await bffApiClient.delete('/cloudinary/deleteMultipleAssets', {
      assets,
      accessToken
    });
  ...
  uploadFormImage: async function (image: File, resourceType: string, folderName = 'whatsnxt-blog') {
    try {
      ...
      const { data } = await bffApiClient.post('/cloudinary/upload-image', formData);
