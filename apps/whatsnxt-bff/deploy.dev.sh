...
if [ "$1" = "--auto" ]; then
  ...
  CURRENT_VERSION=$(grep -oP 'dev_image_version = "\K[^"]*' terraform/terraform.tfvars)
  echo "CURRENT_VERSION: ${CURRENT_VERSION}"
  # Extract the version number after 'whatsnxt-'
  OLD_VERSION=$(echo "$CURRENT_VERSION" | grep -oP '[a-zA-Z]+-v[0-9]+\.[0-9]+$')

  # Split the version into major and minor parts "prod-v4.38"
  FULL_VER=$(echo "$OLD_VERSION" | awk -F'-' '{print $2}')
  VER_MAJOR_WITH_PREFIX=$(echo "$FULL_VER" | awk -F'.' '{print $1}')
  VER_MAJOR=$(echo "$VER_MAJOR_WITH_PREFIX" | awk -F'v' '{print $2}')
  PREFIX=$(echo "$VER_MAJOR_WITH_PREFIX" | awk -F $VER_MAJOR '{print $1}')
  ...
  NEW_VERSION="$ENV_NAME-$PREFIX$VER_MAJOR.$NEW_MINOR"
...
cd terraform
sed -i "s/^dev_image_version = .*/dev_image_version = \"$NEW_VERSION\"/" terraform.tfvars
