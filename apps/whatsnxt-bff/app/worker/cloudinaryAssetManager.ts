...
const cloudinaryAssetManager = async (
  formData: any,
  customFilename: any,
  options: any = {},
) => {
  ...
  return new Promise((resolve, reject) => {
    const workerPath = path.resolve(__dirname, "cloudinary-worker.js");
    ...
    function startWorker() {
      ...
      worker.on("message", (result) => {
        ...
        console.log(
          "Cloudinary Asset Manager :: Worker response:",
          JSON.stringify(result, null, 2),
        );

        if (result && result.error) {
          console.error(
            "Cloudinary Asset Manager :: Worker returned error:",
            result.error,
          );
          ...
          console.log(
            "Cloudinary Asset Manager :: Success response received:",
            result.response,
          );
          ...
          console.error(
            "Cloudinary Asset Manager :: Error response received:",
            result.error,
          );
          reject(new Error(result.error));
        } else {
          ...
          console.log(
            "Cloudinary Asset Manager :: Legacy or unexpected response format:",
            result,
          );
          resolve(result);
        }
      ...
      worker.on("error", (error) => {
        clearTimeout(timeout);
        console.error("Cloudinary Asset Manager :: Worker error event:", error);
        reject(error);
      });
      ...
      worker.on("exit", (code) => {
        ...
        if (code !== 0) {
          console.error(
            "Cloudinary Asset Manager :: Worker exited with code:",
            code,
          );
